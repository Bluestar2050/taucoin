...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vreward.size(); i++)
        {
            ...
            if ( reward.rewardBalance > local && abs(diffAmount) > 20)
            {
...
bool CheckRewards(const CTransaction& tx, CValidationState &state, bool fScriptChecks, unsigned int flags, bool cacheStore)
{
    ...
    if (fScriptChecks) {
        for (unsigned int i = 0; i < tx.vreward.size(); i++) {
            ...
            if (senderRwdInTx > senderRwdInDB && abs(diffAmount2) > 20)
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    int64_t abstemp = block.baseTarget - getNextPotRequired(pindexPrev);
    if (abs(abstemp) > 10)
